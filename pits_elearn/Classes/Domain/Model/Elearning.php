...
class Tx_PitsElearn_Domain_Model_Elearning extends Tx_Extbase_DomainObject_AbstractEntity {
    ...
    public function doGetThema($recordPid = NULL) {
        ...
        $where_clause = 'sys_language_uid = ' . $GLOBALS['TSFE']->sys_language_uid . ' AND pid=' . $recordPid . " AND deleted = 0 AND hidden = 0";
    ...
    public function doGetChapters($themaValue) {
        ...
        $where_clause = 'sys_language_uid = ' . $GLOBALS['TSFE']->sys_language_uid . ' AND thema_id=' . $themaValue . " AND deleted = 0 AND hidden = 0";
        $where_clause .= $enableFields;
        $GLOBALS['TYPO3_DB']->store_lastBuiltQuery = 1;
        $themaInformations = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
                $select_fields, $from_table, $where_clause, $groupBy = '', $orderBy = '', $limit = '', $uidIndexField = '');
    ...
    public function doGetQuesions($whereClause, $questCount, $flexFormQuestCount) {
        if (!empty($_GET['L'])) {
            $GLOBALS['TSFE']->sys_language_uid = $_GET['L'];
        }
        $limitVar = ( $questCount ) ? $flexFormQuestCount : 1;
    ...
    public function doGetResult($request,$configManager = NULL) {
        if (!empty($_GET['L'])) {
            $GLOBALS['TSFE']->sys_language_uid = $_GET['L'];
        }
        $html = '';
        switch ($request['qType']) {
            case 'dragdrop':
                ...
                $html = $this->makeHtml($dragDropRecord,$configManager);
                break;
            case 'freetext':
                $select_fields = '*';
                ...
                $answerText = explode(',', $answerArr[0]['answer_text']);

                $answerArr['answer_text'] = "<span class='green'>" . $answerText[0] . "</span>";
                if (!empty($answerArr[0]['answer_img'])) {
                    $answerArr['answer_text'] = "<span class='green'>" . $answerText[0] . "</span><p>
                		<a id='ansimage' rel='lightbox' data-lightbox='" . $answerArr[0]['answer_img'] . "'
                				href='fileadmin/" . $answerArr[0]['answer_img'] . "' class='lightbox'>" . Tx_Extbase_Utility_Localization::translate('pits_elearn.showImage', 'pitsElearn') . "</a></p>";
                }
    ...
    public function makeHtml($answerArr,$configManager) {
          foreach ($answerArr as $key => $value) {
              $image =  "fileadmin/". $value['dragdrop_answer_img'];
              $imgTSConfig = array();
              $imgTSConfig['file'] = $image;
              $imgTSConfig['file.']['width'] = '290';
              $imgTSConfig['file.']['height'] = '160';
              $imgTSConfig['altText'] = 'preview';
              $answerArr[$key]['dragdrop_answer_img'] = $configManager->IMG_RESOURCE($imgTSConfig);
    ...
    public function doGetMatrixOption($qid) {
        ...
        $whereClause = 'MQ.sys_language_uid = ' . $GLOBALS['TSFE']->sys_language_uid . ' AND MQ.parentid = "' . $qid . '" AND MQ.deleted = 0 AND MQ.hidden = 0';
        $orderByClause = 'RAND()';
        $limitClause = '';
    ...
    public function doGetDragDropOption($qId) {
        ...
        foreach ($dragdropOptions as $key => $value) {
            ...
            $dragdropOptions[$key]['imagePath'] = 'fileadmin/' . $dragdropOptions[$key]['dragdrop_answer_imgRandom'];
        }
