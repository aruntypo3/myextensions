...
class Tx_PitsElearn_Controller_ElearningController extends Tx_Extbase_MVC_Controller_ActionController {
    ...
    public function doGetContent($themaValue) {
        ...
        elseif($themaValue != 0) {
            $content.="<option value='null'><f:translate key='pits_elearn.no_selection'>Nichts ausgew�hlt!</f:translate></option>";
        }else{
            $content.="<option value='0'><f:translate key='pits_elearn.select_chapter'>Kapitel</f:translate></option>";
        }
    ...
    public function detailAction() {
        ...
        $flexFormQuestCount = $this->settings['questionsCount']; 
        ...
        $questionCount = $this->eLearningModel->doGetQuesions($whereClause, $questCount = TRUE, $flexFormQuestCount);
        ...
        $questionRecord = $this->eLearningModel->doGetQuesions($whereClause, $questCount = FALSE, $flexFormQuestCount);
    ...
    public function showResultAction() {
        ...
        $this->storeSession($storeSessionValue);
        $configManager = $this->configurationManager->getContentObject();
        $answerArr = $this->eLearningModel->doGetResult($request,$configManager);
    }
    ...
    public function resultRenderAction() {
        ...
        if (!empty($request['tnt_thema']) && !empty($request['tnt_thema_chapter'])) {
            $currentSesssion['selectedThema'] = $request['tnt_thema'];
            $currentSesssion['selectedChapter'] = $request['tnt_thema_chapter'];
            $sessionViewValue['selectedThema'] = $request['tnt_thema'];
            $sessionViewValue['selectedChapter'] = $request['tnt_thema_chapter'];
            $currentSesssion['currentRandomSeed'] = $storeSessionValue['currentRandomSeed'];
            $currentSesssion['mark'] = '';
            $this->storeSession($currentSesssion);
            $GLOBALS["TSFE"]->fe_user->setKey("ses", "sessionView", $sessionViewValue);
            $GLOBALS["TSFE"]->fe_user->sesData_change = true;
            $GLOBALS["TSFE"]->fe_user->storeSessionData();
            $conf = array(
                'parameter' => $this->settings['targetPage'],
