...
define('TYPO3_MOD_PATH', '../typo3conf/ext/bibus_cms/wizard/');
...
class wizard_table_overview {
	...
	function init()	{
		...
		//$this->doc->backPath = ../;
		//var_dump($this->doc->backPath);exit;
	...
	function getTableHTML($cfgArr,$row)	{
		...
		$tRows=array();
		$k=0;
		foreach($cfgArr as $rowKey => $cellArr)	{
			if (is_array($cellArr))	{
					// Initialize:
				$cells=array();
				$a=0;
				// Traverse the columns:
				foreach($cellArr as $cellkey => $cellContent)	{
					if ($this->inputStyle)	{
						$newCellContent = explode( "->" , $cellContent )  ;
						if ( ( $rowKey  == 0 && $cellkey == 0 ) || (  $rowKey  != 0 &&  $cellkey == 0 ) ){
							$display  = ( $rowKey  == 0 && $cellkey == 0 ) ? 'display:none;' : '';
							$cells[]='<div style="'.$display.'float: left; width: 202px ! important;"><input type="text" style="width:130px" name="TABLE[c]['.(($k+1)*2).']['.(($a+1)*2).']" value="'.htmlspecialchars($newCellContent[0]).'" />
								<input type="text" style="width:50px" name="TABLE[d]['.(($k+1)*2).']['.(($a+1)*2).']" value="'.htmlspecialchars($newCellContent[1]).'" /> </div>';
						}else {
							if ( $rowKey != 0 )
								$width = 'width:20px;';
							$cells[]='<input type="text" style="'.$width.'" name="TABLE[c]['.(($k+1)*2).']['.(($a+1)*2).']" value="'.htmlspecialchars($newCellContent[0]).'" />';
						}
	...
	function cfgArray2CfgString($cfgArr)	{
		...
		foreach ($this->TABLECFG['c'] as $a => $value) {
			...
			foreach ($this->TABLECFG['c'][$a] as $b => $value) {
				$test = $this->tableParsing_quote.str_replace($this->tableParsing_delimiter,'',$this->TABLECFG['d'][$a][$b]).$this->tableParsing_quote;
				$thisLine[] = $this->tableParsing_quote.str_replace($this->tableParsing_delimiter,'',$this->TABLECFG['c'][$a][$b]).$this->tableParsing_quote."->".$test;
...
// Make instance:
$SOBE = t3lib_div::makeInstance('wizard_table_overview');
$SOBE->init();
...
?>
<head>
<link rel="stylesheet" href="<?php echo t3lib_div::getIndpEnv('TYPO3_SITE_URL');?>typo3conf/ext/bibus_cms/wizard/style.css" type="text/css" />
</head>
