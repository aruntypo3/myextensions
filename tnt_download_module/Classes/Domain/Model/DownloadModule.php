...
class DownloadModule extends \TYPO3\CMS\Extbase\DomainObject\AbstractEntity {
    ...
    public function doGetCategory($enableFields, $parent) {
        ...
        $whereClause = " tx_tntdownloadmodule_domain_model_parentcategory.parent = " . $parent ." AND tx_tntdownloadmodule_domain_model_parentcategory.sys_language_uid = 0".$enableFields;
        ...
        $i = 0;
        $categoryList = array();
        foreach ( $catInformations as $catId => $catValue ) {
        	if( $lang > 0 ){
        		...
        		$from_table = 'tx_tntdownloadmodule_domain_model_parentcategory AS tx_tntdownloadmodule_domain_model_parentcategory';
        		$whereClause = " tx_tntdownloadmodule_domain_model_parentcategory.parent = " . $parent ." AND tx_tntdownloadmodule_domain_model_parentcategory.sys_language_uid = " . $lang ." AND tx_tntdownloadmodule_domain_model_parentcategory.l10n_parent = ".$catValue['parentId'].$enableFields;
        		$GLOBALS['TYPO3_DB']->store_lastBuiltQuery = 1;
        		$catTransInformations = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows($select_fields, $from_table, $whereClause, $groupBy = '', $orderBy = 'tx_tntdownloadmodule_domain_model_parentcategory.sort_order ASC', $limit = '', $uidIndexField = '');
        		
        	}
        	if( sizeof( $catTransInformations[0] ) > 0 ){
        		$categoryList[$i]['parentId'] .= $catTransInformations[0]['parentId'];
        		$categoryList[$i]['category_title'] .= $catTransInformations[0]['category_title'];
        		$categoryList[$i]['l10n_parent'] .= $catTransInformations[0]['l10n_parent'];
        	}else{
        		$categoryList[$i]['parentId'] .= $catValue['parentId'];
        		$categoryList[$i]['category_title'] .= $catValue['category_title'];
        		$categoryList[$i]['l10n_parent'] .= $catValue['l10n_parent'];
        	}
        	$i++;
        }
    ...
    public function doGetDetails($enableFields, $request, $flexSettings = '') {
        ...
        $lang = $GLOBALS['TSFE']->sys_language_uid;
        //echo $lang;
        $langCond = ' AND tx_tntdownloadmodule_domain_model_downloadmodule.sys_language_uid = 0';
        if( $lang > 0 ){
          $langCond = " AND (tx_tntdownloadmodule_domain_model_downloadmodule.sys_language_uid = ".$lang." OR  tx_tntdownloadmodule_domain_model_downloadmodule.sys_language_uid = 0 )";
        }
        ...
        $whereClause = $whereCond2 . "sfr.tablenames = 'tx_tntdownloadmodule_domain_model_downloadmodule' AND sfr.fieldname = 'documnet_file_field' AND sfr.deleted=0 AND sfr.hidden = 0 AND tx_tntdownloadmodule_domain_model_downloadmodule.documnet_file_field > 0 AND (" . $whereCond . ")".$langCond.$enableFields;
        $GLOBALS['TYPO3_DB']->store_lastBuiltQuery = 1;
    ...
    public function doGetAllDocuments($singleInformations, $request, $per_page) {
        ...
        foreach ($singleInformations as $key => $value) {
            ...
            $singleInformations[$key]['size'] = round($value['size'] / 1024);
            $singleInformations[$key]['extension'] = $value['extension'];
